...
class HtmlReport:
    ...
    def write(self, output_path, records, metric_keys, version="v2.0",
        ...
        #   • Chart.js (charts)
        #   • simple-datatables (tables w/ pagination & search)
        #   • html2canvas + jsPDF (image/PDF export)
        #   • Lots of semantic <div> sections and CSS variables for theming
        html = """<!DOCTYPE html>
...
/* Header gradient bar */
.header-gradient {{
  background: linear-gradient(135deg, var(--glacier-ice), var(--glacier-mid) 40%, var(--glacier-navy));
  color: #fff;
}}
...
/* Subtle vertical spacing so pills don’t touch */
.sv-pill {
  margin: 2px auto;
}
  ...
  <div class="accordion section-toggle mb-3" id="mainAccordion">
          ...
          <div class="row mt-3 g-3 align-items-center">
            <div class="col">
              <label class="small">Accuracy
                <span class="metric-info" data-bs-toggle="popover" data-bs-trigger="hover focus" title="Accuracy" data-bs-content="Proportion of total predictions that were correct. (TP + TN) / Total">?</span>
            ...
            <div class="col">
              <label class="small">Recall
                <span class="metric-info" data-bs-toggle="popover" data-bs-trigger="hover focus" title="Recall" data-bs-content="Of all truly vulnerable cases, how many were correctly identified. TP / (TP + FN)">?</span>
    ...
    <div class="accordion-item">
      ...
      <div id="collapseChart" class="accordion-collapse collapse show" aria-labelledby="headingChart">
        <div class="accordion-body">
          <div class="chart-container mb-2"><canvas id="metricChart"></canvas></div>
        </div>
      </div>
    </div>
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table id="results-table" class="table table-sm table-hover table-bordered small">
                  <thead>
                    <tr>
                      <th>Dataset</th><th>Model</th><th>Prompt</th><th>Lang</th>
                      <th title="Accuracy: (TP+TN)/Total">Acc</th>
                      <th title="Precision: TP/(TP+FP)">Prec</th>
                      <th title="Recall: TP/(TP+FN)">Rec</th>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <thead class="table-light">
                        <tr>
                        <th>ID</th><th>Dataset</th><th>Lang</th><th>Model</th><th>Prompt</th>
...
function applyFilters(){
  ...
  const res=RECORDS.filter(r=>
    (!d||r.dataset===d)&&(!m||r.model===m)&&(!p||r.prompt===p)&&
    (r.Accuracy>=acc)&&(r.Precision>=pre)&&(r.Recall>=rec)&&(r.F1>=f1)
  );
  ...
  parts.push(`<span class='text-secondary ms-2'>(${res.length}/${RECORDS.length})</span>`);
...
/* ----------------------------------------------------------------------------
   LEADERBOARD
   For each metric (Acc/Prec/Rec/F1), show top 3 rows with a small bar.
---------------------------------------------------------------------------- */
function renderLeaderboard(data){
  const body=document.getElementById('leaderboardBody');
  const metrics=[{k:'Accuracy',c:'#0d6efd'},{k:'Precision',c:'#ffc107'},{k:'Recall',c:'#20c997'},{k:'F1',c:'#dc3545'}];
  if(!data.length){ body.innerHTML='<div class="text-muted">No data for leaderboard.</div>'; return; }

  body.innerHTML = metrics.map(({k,c})=>{
    const sorted=[...data].sort((a,b)=>b[k]-a[k]).slice(0,3);
    const max = sorted.length? sorted[0][k] : 1;
    const items = sorted.map((r,i)=>{
      const pct = max>0 ? (r[k]/max*100) : 0;
      const medal = ['🥇','🥈','🥉'][i] || '';
      return `
        <div class="d-flex align-items-center mb-2">
          <div class="me-2" style="width:22px;">${medal}</div>
          <div class="flex-grow-1">
            <div class="d-flex justify-content-between small">
              <div><b>${r.model}</b> <span class="text-muted">(${r.prompt})</span> <span class="text-muted">[${r.dataset}]</span></div>
...
function renderTable(data) {
  ...
  // Build cache using the full dataset (RECORDS), not `data`, so the column
  // filter menus always contain every available value.
  RESULTS_ALL = RECORDS.map(r => {
    const cols = [
      r.dataset,
      r.model,
      r.prompt,
    ...
    const html = `
      <tr title="Dataset: ${r.dataset}\nModel: ${r.model}\nPrompt: ${r.prompt}\nAcc: ${roundNum(r.Accuracy)}\nPrec: ${roundNum(r.Precision)}\nRec: ${roundNum(r.Recall)}\nF1: ${roundNum(r.F1)}">
        ...
        <td>${cols[4]}</td><td>${cols[5]}</td><td>${cols[6]}</td><td>${cols[7]}</td>
      </tr>`;
    return { cols, html };
  });

  // Insert ALL rows initially (filters will pare it down later)
...
function renderChart(data) {
  ...
  data.forEach(r => {
    ...
    if (!groups[key]) {
      ...
      groups[key].Accuracy += r.Accuracy || 0;
      groups[key].Precision += r.Precision || 0;
      groups[key].Recall += r.Recall || 0;
      groups[key].F1 += r.F1 || 0;
      groups[key].count += 1;
    }
  });

  const combined = Object.values(groups).map(g => ({
    label: `${g.model} (${g.prompt})`,
  ...
  const datasets = [
    {
      label: 'Accuracy',
      data: combined.map(d => d.Accuracy),
      backgroundColor: '#0d6efd'
    },
    {
      label: 'Precision',
      data: combined.map(d => d.Precision),
      backgroundColor: '#ffc107'
    },
    {
      label: 'Recall',
      data: combined.map(d => d.Recall),
      backgroundColor: '#20c997'
    },
    {
      label: 'F1',
      data: combined.map(d => d.F1),
...
document.addEventListener('DOMContentLoaded',()=>{
  document.getElementById('btn-reset').addEventListener('click', ()=>{
    ['filterDataset','filterModel','filterPrompt'].forEach(id=>document.getElementById(id).value='');
...
---------------------------------------------------------------------------- */
function exportCSV(){
  const rows=[['Dataset','Model','Prompt','Lang','Acc','Prec','Rec','F1']];
  applyFilters().forEach(r=>rows.push([r.dataset,r.model,r.prompt,r.language||'',r.Accuracy,r.Precision,r.Recall,r.F1]));
...
/* Glossary PDF (Color scheme A: Glacier Blue accents) */
function exportGlossary(){
  ...
  pdf.setFontSize(16);
  pdf.text('Metrics Glossary — Glacier', 10, 13);
  ...
  section('Accuracy', 'Proportion of total predictions that were correct. Formula: (TP + TN) / (TP + TN + FP + FN). Use when overall correctness matters (balanced classes).');
  ...
  section('Recall', 'Of all truly vulnerable samples, how many were correctly identified. Formula: TP / (TP + FN). Use when missing a vulnerability is costly (safety-critical).');
...
document.addEventListener('DOMContentLoaded', ()=>{
  populateFilters();
  ['filterDataset','filterModel','filterPrompt'].forEach(id=>{
...
function renderDetails(data){
  ...
  data.forEach(rec => {
    (rec.predictions || []).forEach(p => {
      ...
      const cols = [
        p.id, rec.dataset, lang, rec.model, rec.prompt,
        (p.pred || "").toUpperCase(),
        (p.gold || "").toUpperCase(),
        icon
      ];
      ...
      DETAILS_ALL.push({ cols, html });
    });
  });

  tbody.innerHTML = DETAILS_ALL.map(x => x.html).join("") || "<tr><td colspan='8' class='text-muted'>No predictions available.</td></tr>";
  const dc = document.getElementById('detailCount');
  if(dc) dc.textContent = `Showing ${total} predictions`;
}
...
function addColumnFilters(tableSelector, instance) {
  ...
  headerCells.forEach((th, colIndex) => {
    ...
    if (tableSelector === "#results-table" && colIndex >= 4) {
    ...
    const values = new Set(bag.map(r => (r.cols[colIndex] || "").toString()));
    ...
    select.addEventListener("change", e => {
      ...
      if (tableSelector === "#results-table") {
        ...
        if (rc) rc.textContent = `Showing ${filtered.length} / ${RESULTS_ALL.length}`;
      } else {
        const dc = document.getElementById('detailCount');
...
function resetColumnFilters(tableSelector, instance, activeMap) {
  ...
  if (tableSelector === "#results-table") {
    const rc = document.getElementById('recordCount');
    if (rc) rc.textContent = `Showing ${bag.length} / ${RESULTS_ALL.length}`;
  } else {
    const dc = document.getElementById('detailCount');
...
function initResultsTable() {
  ...
  resultsTable = new simpleDatatables.DataTable("#results-table", {
    ...
    sortable: false,       // we handle sorting visually elsewhere
  ...
  const rc = document.getElementById('recordCount');
  if (rc) rc.textContent = `Showing ${RESULTS_ALL.length} / ${RESULTS_ALL.length}`;
}
...
function updateResultsTable(data) {
  ...
  // Keep only those rows that appear in `data` (based on dataset+model+prompt)
  const filtered = RECORDS.filter(r =>
    (!data.length || data.some(d =>
      d.dataset === r.dataset && d.model === r.model && d.prompt === r.prompt))
  );
  ...
  const rc = document.getElementById('recordCount');
  if (rc) rc.textContent = `Showing ${newRows.length} / ${RESULTS_ALL.length}`;
}
...
function updateDetailTable(data) {
  ...
  // Flatten all predictions from all currently-active records
  data.forEach(rec => {
    (rec.predictions || []).forEach(p => {
      const icon = p.pred === p.gold ? "✅" : "❌";
      const predBubble = p.pred === "safe"
        ? `<span class="sv-pill sv-safe">SAFE</span>`
        : p.pred === "vulnerable"
          ? `<span class="sv-pill sv-vuln">VULNERABLE</span>`
  ...
  const dc = document.getElementById('detailCount');
  if (dc) dc.textContent = `Showing ${newRows.length} predictions`;
}
